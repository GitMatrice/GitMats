...
This document provides practical workflows and examples for using GitMats with various versioning backends. The primary CLI tool is `gmt` (GitMats CLI).
...
#
# To start working:
#   cd ~/.gitmats/workspaces/agent-exp-1/workspace
...
# Connection string unchanged: postgres://db_abc123.dbay.cloud...
...
#
# AI Summary:
#   Added users table with email, password_hash, created_at columns.
#   Created user_sessions table for session management.
#   Added indexes for email lookup and primary keys.
...
# LakeBase branch gitmats-my-project retained (use --cleanup-lakebase to delete)
```
